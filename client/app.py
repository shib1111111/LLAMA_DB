...
def signup(username, password):
    response = requests.post(f"{SERVER_URL}/signup", json={"username": username, "password": password})
    if response.status_code == 200:
        st.session_state.access_token = response.json()["access_token"]
...
def generate_api_key():
    ...
    response = requests.post(f"{SERVER_URL}/api_key", headers=headers)
    if response.status_code == 200:
...
def list_api_keys():
    headers = {"Authorization": f"Bearer {st.session_state.access_token}"}
    response = requests.get(f"{SERVER_URL}/api_keys", headers=headers)
    if response.status_code == 200:
...
def query_db(query):
    try:
        ...
        response = requests.post(f"{SERVER_URL}/query", headers=headers, json=payload)
        if response.status_code == 200:
